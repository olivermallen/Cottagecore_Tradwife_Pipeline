...
# hardcoded directories, probably want to make them parameters in final code
...
def addpost(post, dataframedict):
    ...
    image_urls = []
    links = []  # extracting links from body of post as well
    if "body" in post:
        ...
        for photo in post["photos"]:
            soup = BeautifulSoup(photo["original_size"]["url"], features="lxml")
            images = soup.find_all("img")
            for image in images:
                image_urls.append(image.get("src"))
    if "reblog" in post:
        soup = BeautifulSoup(post["reblog"]["comment"], features="lxml")
        images = soup.find_all("img")
        for image in images:
            image_urls.append(image.get("src"))
    dataframedict["image_urls"].append(image_urls)
    dataframedict["links"].append(links)
    # now download images
    # i = 0
...
df.to_pickle(
    "/home/mel/Desktop/repos/cottagecore_tradfem_pipeline/full_data2.pkl",
    compression="infer",
    protocol=5,
    storage_options=None,
)
df.to_csv("/home/mel/Desktop/repos/cottagecore_tradfem_pipeline/full_data2.csv")
